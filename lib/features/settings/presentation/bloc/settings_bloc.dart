...
class SettingsBloc extends Bloc<SettingsEvent, SettingsState> {
  ...
  Future<void> _onToggleSound(ToggleSoundEvent event, Emitter<SettingsState> emit) async {
  ...
  Future<void> _onTapDeveloper(TapDeveloperEvent event, Emitter<SettingsState> emit) async {
    ...
    if (newCount >= 10) {
      ...
      emit(current.copyWith(
        developerTapCount: newCount,
        toastMessage: 'You are $remaining tap(s) away from finding the developer.',
      ));
    } else {
  ...
  Future<void> _onDeleteAiModel(DeleteAiModelEvent event, Emitter<SettingsState> emit) async {
    if (state is! SettingsLoadedState) return;
    final current = state as SettingsLoadedState;
